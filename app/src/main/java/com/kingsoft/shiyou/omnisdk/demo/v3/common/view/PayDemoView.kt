...
class PayDemoView : DemoView, IPayCallback {
    ...
    @Volatile
    private var productSkuType: OmniSDKProductType = OmniSDKProductType.INAPP
    ...
    override fun initView() {
        ...
        roleLevelEt
        roleVipLevelEt
        serverKeyEt = findViewById(R.id.v3_pay_demo_view_server_key_edt)
        R.id.v3_pay_demo_view_create_cno_btn.addClickListener {
            generateGameCno()
            appView.showMessageDialog("订单号: $gameCno", "创建成功")
        }
        R.id.v3_pay_demo_view_pay_btn.addClickListener {
            pay()
        }
        generateGameCno()
    }
    ...
    private fun initProductSkuTypes() {
        ...
        OmniSDKProductType.values().forEach {
            skuTypes.add(it.name)
        }

        val skuTypesSpinner = findViewById<Spinner>(R.id.v3_pay_demo_view_product_type_spinner)
        ...
        skuTypesSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(
                ...
                (skuTypesSpinner.selectedItem as? String)?.let {
                    productSkuType = OmniSDKProductType.valueOf(it)
    ...
    override fun onSucceeded(orderId: String) {
        MainScope().launch {
            cancelProcessingDialog()
        }
        appView.showMessageDialog("订单号: $orderId", "支付成功")
    }
    ...
    override fun onFailed(error: OmniSDKError) {
        ...
        // 注意 4005 不要也不需要向用户展示，用户只需看到GP的提示就行；
        // 因为GP会在一段时间内重复发送这个code类型，但其实只有一个订单，
        // 游戏应该在用户订阅过一次后，隐藏对应的订阅商品
        Log.e("Pay", "payError = ${error.getDescription()}")
